...
<ol>
  ...
  <li>真正的关键是大规模集成5G射频系统芯片.
  	  <ul>
  	      <li>a. 2016 ISSCC, session 9.1, TI发表了4G基站射频系统的芯片设计. 这个架构把射频信号直接转换到零中频, 转成基带数字信号. 为了消除镜像, 再加一条接收回路把镜像复制出来, 用在以后消除. 
              这个架构是今天以CDMA为基础的射频收发系统的标准架构, 特别适合用大规模集成电路来实现. 我记得20年前的书本就已经提到过, 没有商业机密或是侵犯专利的问题. 
              <br />文章和投影片里面的数据讲解的还是蛮清楚的, 没有解释的部分, 也可以从其他的文章里追踪出来, 或是用计算机模拟出来, 而且线路设计并不那么困难. 
              <br />它有两个接收信道和两个发送信道用来实现MIMO. 用的是45纳米CMOS制作工艺. 芯片尺寸是七毫米乘七毫米.</li>
          <li>b. 上面谈到的那一篇文章, 可以作为5G基站芯片的基本架构, 从4G到5G改变了什么? 
          	  <br />在2018年ISSCC session 4.5卡内基梅龙大学发表的文章里, 有5G频谱以及带宽的规格. 世界各国包括美国主要都在25GHz到29.5GHz之间. 美国还有一段从37到40GHz.
              这些代表着LNA, MIXER, VCO, PLL等等的工作频率会跟着提高. ADC, DAC, 数字滤波器也会跟着带宽和原始码流而加快. 这些都不是问题, 在过去三年的ISSCC文章里, 都有在高频率的种种设计, 用SPICE模拟就行了.
              <br />还有文章谈到在发射前作数字处理, 控制并减少功率放大器的直流功耗. 
              <br />5G在去年底还出现了一个新标准, 那就是用相控数组天线技术, 把无线电波束对准多个用户. 因为能量集中信号自然就加强了很多. 
              <br />2018年ISSCC在SESSION 4, SESSION 9有许多公司和大学发表的有关这方面的文章和产品以及实验结果. 有ADI, QUALCOMM, BROADCOM, 哥伦比亚大学, 乔治亚理工学院, 华盛顿州立大学, 日本东芝, 诺基亚. 
              <br />ADI还发表的一颗把2G, 3G, 4G, 5G射频部分集成在一颗芯片上的文章, 它号称是用在无线基站, 我看起来更像是用在5G手机里面.
              因为手机更需要把老的通讯标准集成进去, 否则5G基站没有充分设立的时候, 没有人会去买只有5G的手机. 
              <br />新加坡南洋理工还用天线数组技术, 把合成孔径雷达集成在一个芯片上.
  ...
  <li>无线本地环路802.11ax. Broadcom在2018 ISSCC session 4.2的文章发表了802.11ax芯片组, 射频部分用40纳米CMOS. 基带中频部分用28纳米CMOS. 它可以用多个射频部分组成相控天线数组的基站. 
  	  <br />日本东芝在2018 ISSCC session 28.1的文章里发表了802.11ax单一芯片. 用28纳米CMOS, 芯片的尺寸是6.6毫米x6.6毫米. 
  	  <br />在这个标准里使用了相控数组雷达的技术. 性能高. 终端芯片的成本会很低. 我相信这个技术会代替电缆, 电话线, cable modem, DSL modem, 就好像wifi取代的以太网线一样.</li>
  <li>建议在清华复旦旁边建立开发这种芯片的公司. 并请有经验的技术人员到学校开课, 同时训练学生以及其他教授. 学生在这个项目的工作可以变成学分. 教授们也可以在这项目得到研究课题和经费. 
  	  ...
  	  <br />微波射频线路的设计, 在国外早已是公开的数据, 而这些正是国内大学的教育盲点. 可以公开讨论相互切磋. 这样才能进步. 藏着掖着, 把什么都当成宝贝供着, 只会减慢进步的步伐. 
