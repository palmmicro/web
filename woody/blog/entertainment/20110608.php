...
<ol>
  <li>Enable WDS function.
  <li>Set up SSID(to be bridged) and BSSID(to be bridged) the AP of the WiFi network you are going to work with, the "Search" button can be used to search for those value, especially the BSSID.
  <li>Set up the rest of WiFi options exactly as the AP, including channel, mode and security. Actually you need to set up the same security parameters twice,
      in both the "Wireless Settings"->"WDS" section and the "Wireless Security" section.
  <li>Test and verify the bridge function is working.
  <li>Disable DHCP server function on this bridge and set its local IP address to an address outside of the original DHCP server range.
      For example, set your original DHCP client address from 192.168.1.100 to 192.168.1.199 and set this bridge to 192.168.1.99. In this way, any device connected on this bridge will still use the original DHCP server.
</ol>
<br />What if you have another wireless router in hand but it does not have function like "WDS"? <a href="https://openwrt.org/" target=_blank>OpenWrt</a> is where you should check,
