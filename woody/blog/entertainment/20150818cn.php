...
除此以外, 在美股休市的日子里, 它也比官方估值更能反映实际的净值. 至于为什么叫它参考估值, 那是因为我实在不知道给它取什么名字好了. 
事实上, 在英文代码中我给它取名为<b>Fair Est</b>, 意思是一个公平的估值. 
<br />官方估值中使用^SPSIOP是为了100%准确的预测当晚要公布的官方净值, 而参考估值和实时估值使用XOP的原因是因为它是实际用来交易的品种, 
使用XOP计算出来的各种均线值可以直接用在我自己的套利交易中.
<br />在美股交易时段, 这3个估值通常都是完全一致的. 如果偶尔出现官方估值和参考估值不同, 那是因为^SPSIOP和XOP的数据没能在同一分钟内自动校准.
...
<ol>
  <li>单独只做申购赎回套利的, 看官方估值.</li>
  <li>做跟XOP配对交易的, 看参考估值.</li>
...
<blockquote><code>echo UrlGetQueryDisplay('symbol').($bChinese ? '净值和收盘价历史比较' ： ' NAV Close History Compare');</code></blockquote>
