...
<ol>
  <li>开启WDS功能. 
  <li>把(桥接的)SSID和(桥接的)BSSID的值设置成你要加入工作的WiFi网络中AP的值. 用"扫描"键可以方便的找到这些值, 尤其是BSSID. 
  <li>把剩下的WiFi相关选项都设置成跟AP一致, 包括信道、模式和加密等. 事实上你需要把同样的加密参数设置2次, 一次在"基本设置"->"WDS"选项下面, 另外一次在"无线安全设置"部分. 
  <li>测试确认网桥功能正常. 
  <li>关闭你的新网桥上的DHCP服务器, 并且把它的IP地址设置到一个老DHCP服务器不使用的地方. 例如, 设置你的老DHCP服务器使用从192.168.1.100到192.168.1.199用于分配地址, 把这个新网桥的地址设置成192.168.1.99.
      这样任何接到网桥上的设备都可以依旧使用原来的DHCP服务器. 
</ol>
<br />如果你手头能找到的无线路由器没有类似于"WDS"的功能, 那你应该去<a href="https://openwrt.org/" target=_blank>OpenWrt</a>看看. 上面有给各种路由器的开放源代码的软件, 其中包括把无线路由器改为Ethernet转WiFi网桥的功能. 
