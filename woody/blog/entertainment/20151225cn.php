...
<br />去掉前后双引号后, 按逗号','分隔的各个字段意义如下表.
<?php
    EchoInterpretationTable(array(array('0', '31.85', '当前价格'),
...
<p>2016年2月16日
<br />晚上9点多的时候, uqot跟我说<a href="20150818cn.php">华宝油气</a>净值计算出问题了. 我看了一下调试信息, 发现8点的时候做了一次自动校准,
从<a href="http://hq.sinajs.cn/list=f_162411" target=_blank>http://hq.sinajs.cn/list=f_162411</a>拿到后记录在<b><a href="/debug/sina/f_162411.txt" target=_blank>f_162411.txt</a></b>中的数据如下:
<br /><font color=grey>var hq_str_f_162411="华宝兴业标普油气上游股票(QDII-LOF),
0.406,0.406,0.435,2016-02-15,66.2444";</font>
<br />按照php/<b>_lof.php</b>中的自动校准流程, 拿到15号的华宝油气净值后先跟15号XOP的收盘值校准, 但是昨天美股因为总统日没有交易, 这一步没有成功.
...
<br />去掉前后双引号后, 按逗号','分隔的各个字段意义如下表.
<?php
    EchoInterpretationTable(array(array('0', '华宝兴业标普油气上游股票(QDII-LOF)', 'GB2312编码的名字'),
                                   array('1', '0.406', '目前净值'),
                                   array('2', '0.406', '累计净值'),
...
从<a href="http://hq.sinajs.cn/list=hk02828" target=_blank>http://hq.sinajs.cn/list=hk02828</a>拿到后记录在<b><a href="/debug/sina/02828.txt" target=_blank>02828.txt</a></b>中的数据如下:
...
<br />去掉前后双引号后, 按逗号','分隔的各个字段意义如下表.
<?php
    EchoInterpretationTable(array(array('0', 'HS H-SHARE ETF', '英文名字'),
...
<br />目前从<a href="http://hq.sinajs.cn/list=AU0" target=_blank>http://hq.sinajs.cn/list=AU0</a>拿到后记录在<b><a href="/debug/sina/auo.txt" target=_blank>au0.txt</a></b>中的数据如下:
...
<br />跟<a href="http://finance.sina.com.cn/futures/quotes/AU0.shtml" target=_blank>http://finance.sina.com.cn/futures/quotes/AU0.shtml</a>的显示数据比较, 去掉前后双引号后, 按逗号','分隔的各个字段意义如下表.
<?php
    EchoInterpretationTable(array(array('0', '黄金连续', '名字'),
...
<p>2016年10月17日
<br />最近在股票交易中对<a href="../../res/smacn.php">均线</a>的依赖越来越严重, 而同时Yahoo的负面用户体验和甩卖消息越来越多, 让我比较担心哪一天Yahoo突然不提供历史数据了, 毕竟是一个非公开的东西.
于是下决心自己开始记录历史数据, 刚开始改程序就发现去年底写的读取新浪实时美股数据的代码中缺Yahoo历史数据中提供的的开盘价, 最高价, 最低价和成交量. 又要重新开始看数据接口的字段意义.
从<a href="http://hq.sinajs.cn/list=gb_xop" target=_blank>http://hq.sinajs.cn/list=gb_xop</a>拿到后记录在<b><a href="/debug/sina/xop.txt" target=_blank>xop.txt</a></b>中的数据如下:
...
<br />跟<a href="http://stock.finance.sina.com.cn/usstock/quotes/XOP.html" target=_blank>http://stock.finance.sina.com.cn/usstock/quotes/XOP.html</a>的显示数据比较, 去掉前后双引号后, 按逗号','分隔的各个字段意义如下表.
<?php
    EchoInterpretationTable(array(array('0', '油气开采指数ETF-SPDR S&P', 'GB2312编码的中文名字'),
