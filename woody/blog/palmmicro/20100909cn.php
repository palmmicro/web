...
<ol>
  <li><a href="../../../ar1688/softwarecn.html">下载</a>一份AR1688软件API. 
  <li>把下载文件解压缩到c:\sdcc. 
  <li>进入命令行下c:\sdcc\bin. 
  <li>执行命令: "getopt xxx.xxx.xxx.xxx". 
  <li>修改options.txt文件中"admin_pin"(保存密码位置)的部分. 
  <li>执行命令: "setopt xxx.xxx.xxx.xxx". 
</ol>
<br />getopt.bat和setopt.bat批处理文件中都调用tftp.exe, 要确认Windows系统中安装了它. 
...
<ol>
  <li>设置好"片上IP地址"后点击PalmTool的"话机设置"直接连接设备. 如果可以连接上, 就可以在设置对话框中直接修改. 
  <li>当"调试级别"选项是关闭的时候, PalmTool连不上设备, 用户会得到如"Can not connect to Palm1"这样的提示. 
...
<br />忘记了我们网站<a href="../../../account/logincn.php">登录</a>密码? 访问密码<a href="../../../account/remindercn.php">提醒</a>页面即可. 新的密码会发送到注册电子邮箱.  
<br />为什么不简单的送回原来的密码, 而是要新产生一个呢？因为我们并没有保存用户密码, 保存在数据库中用户密码项的数据是根据密码产生的MD5加密串, 理论上这个加密是不可逆的, 就是说不能从加密串再反过来得到原始串. 
<br />最后我要承认, 虽然这篇网络日志部分来说是被某个寻找AR1688密码的客户所激发, 我的主要目的还是想宣传<a href="../entertainment/20100905cn.php">注册</a>我们的网站帐号并发表网络日志评论.
这也是为什么它被放到了"Palmmicro"的分类中. 
</p>
</div>
