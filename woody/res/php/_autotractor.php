...
{
	EchoHtmlElement($str, 'ol');	// missing 'li' here
}
...
function _echoAutoTractor()
{
	...
	$str .= '仅需用到x86版本的AutoIt3.exe文件. 一定要小心软件来源, 千万不要运行来历不明的.exe文件.';
    EchoOrderList($str);
	...
	$str .= GetFontElement('<br />废弃电脑前，要记得清除全部客户号记录，避免泄露。');
...
function EchoAll()
{
	...
	_echoAutoTractor();
    $acct->EchoLinks();
    echo GetKnownBugs(array('银河证券官网现在的海王星独立交易升级了，导致模拟输入不匹配。老用户先别升级，新用户可以暂停测试。目前0.50版本会自动取消升级提示。',
    						 '网速很重要！在目前代码中有大量模拟按键或者鼠标后等待一秒的被动行为，在网速慢的时候会因为等待时间不够长而出错。我就经常需要在运行代码前先手工把电脑上的网络从天威宽带切换到自己手机上的移动4G热点。',
    						 '在小屏幕笔记本上，显示设置的'.GetInfoElement('缩放与布局').'中，'.GetInfoElement('更改文本、应用等项目的大小').'的选项缺省不是100%。这时AutoIt自带的WinGetPos函数不会跟着调整倍数，导致找不到验证码位置。',
    						 '银河0开头的客户号，需要把开头的0去掉，否则登录后匹配不到主窗口。',
...
{
  	$str = '利用PC上的AutoIt脚本工具软件实现华宝油气(SZ162411)等场内基金拖拉机账户的自动化登录、申购和卖出，提高几万套利党人的时间效率。';
